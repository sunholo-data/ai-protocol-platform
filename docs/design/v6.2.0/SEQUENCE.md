...
- **Event-driven skills** — `SkillTrigger` abstraction (`scheduled` / `pubsub`), Pub/Sub + Cloud Scheduler + worker-job pattern, output routes to any registered channel. Drives Shepherd's `contract-watch` and a tutoring fork's `stuck-detection`
