...
| 4.2 | *deployment demo readiness (deployment-private)* | **P0** | ~4d | client-tenant-management (v6.3.0 ✅), branding.ts (✅), multi-surface (2.9 ✅), 4.1 M1+M4 | A deployment-specific commercial demo sprint. Its transferable result — that a **fork-by-config** deployment (per-deployment branding via `NEXT_PUBLIC_BRAND_*`, an enabled-skills filter, and skill templates) can match a code fork on demo quality — is what the template inherits; see [fork-convergence.md](../v6.6.0/fork-convergence.md). The sprint doc itself is deployment-private. |
...
| 4.2 | *deployment demo readiness (deployment-private)* | shipped 2026-06-08/09 |
...
v6.3.0 complete (3.1 ✅ + 3.2 ✅)
   ...
   └── ACTION-TRIGGER M1-M3 (2026-06-08 ✅)
            ...
            │
            └──► deployment demo readiness (4.2, deployment-private)  [parallel — fixed deadline]
                     │
